...
function Controls({ 
  ...
  return (
    <div className="controls">
      <label>
        Language
        <select className="language-selector" value={locale} onChange={e => setLocale(e.target.value)}>
          {locales.map(l => <option key={l.code} value={l.code}>{l.label}</option>)}
        </select>
      </label>
      <label>
        Seed
        <input value={seed} onChange={e => setSeed(e.target.value.replace(/\D/g, ''))} type="text" />
      ...
      <button className="export-btn" onClick={onExport}>Export to CSV</button>
    </div>
  );
}
...
function App() {
  ...
  useEffect(() => {
    ...
    const cleanup = fetchBooks(1, true);
    return cleanup;
  ...
  function handleExportCSV() {
    ...
    a.href = url;
    a.download = `books_data_${seed}_${locale}.csv`;
  ...
  return (
    <div className="App">
      <h1>Bookstore Data Generator</h1>
